...
from src.migrations import METADATA, db
...
class User(db.Model):
    ...
    block_comment = sa.Column("block_comment", sa.String(300), nullable=True)
    language = sa.Column(
    ...
    paid_till_updated_at = sa.Column(
        "paid_till_updated_at", sa.TIMESTAMP, nullable=True
    )
    ...
    is_vip_issued = sa.Column("is_vip_issued", sa.TIMESTAMP, nullable=True)
    currency_id = sa.Column("currency_id", sa.ForeignKey("currency.id"))
    telegram = sa.Column("telegram", sa.String(64), nullable=True)
    dreams_to_activate = sa.Column(
        "dreams_to_activate", sa.Integer, server_default=sa.text("0")
    )
    ...
    public_email = sa.Column("public_email", sa.String(128), nullable=True)
    about_me = sa.Column("about_me", sa.String(750), nullable=True)
    pipedrive_id = sa.Column("pipedrive_id", sa.Integer, nullable=True)
    apple_id = sa.Column("apple_id", sa.String(2048), nullable=True)

    def __str__(self) -> str:
        """Текстовое представление для логов."""
        return f"User(id={self.id}, verified_email={self.verified_email})"
...
blacklist = sa.Table(
    ...
    sa.Column("jti", sa.String(32), nullable=False, index=True),
)
