...
async def reminder_events():
    ...
    time_add_two_week = (time + timedelta(days=16)).date() #через 2 дня будет ивент и через 2 недели будет повтор поэтому 14+2=16
    ### Проверка на наличия энергии по времени для подписчиков ###
    info_event = await Events.query.where(Events.event_date == time_add_two_days).gino.all()
    ...
    if info_event:
        ...
        for event in info_event:
            text = str(event.description).replace('_', '\_').replace('*', '\*').replace('[', '\[').replace(']','\]').replace(
                '(', '\(').replace(')', '\)').replace('`', '\`').replace('~', '\~').replace('>', '\>').replace(
                '#', '\#').replace('+', '\+').replace('=', '\=').replace('-', '\-').replace('|', '\|').replace(
                '{', '\{').replace('}', '\}').replace('.', '\.').replace('!', '\!')
            for user in info_user:
                await send_msg_mv2(user_id=user.user_id, text= f"*Через два дня {text}*")
