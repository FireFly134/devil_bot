    ...
    heroes = await HeroesOfUsers.query.where(
        and_(
            or_(
                ...
                HeroesOfUsers.time_collection_energy == int(time2_energy.strftime('%H')),
                HeroesOfUsers.time_collection_energy == int(time3_energy.strftime('%H'))
            ),
            HeroesOfUsers.subscription_energy
        )
    ).gino.all()
    for hero in heroes:
        name = str(hero.name).replace('_', '\_').replace('*', '\*').replace('[', '\[').replace(']','\]').replace(
            '(', '\(').replace(')', '\)').replace('`', '\`').replace('~', '\~').replace('>', '\>').replace(
            '#', '\#').replace('+', '\+').replace('=', '\=').replace('-', '\-').replace('|', '\|').replace(
            '{', '\{').replace('}', '\}').replace('.', '\.').replace('!', '\!')
        await send_msg_mv2(user_id=hero.user_id, text=f"*Зайди в игру и забери халявную энергию*\. \({name}\)")
